...
import { HeaderModalNoMetamask } from "./components/HeaderModal/HeaderModalNoMetamask";
...
const App = () => {
  ...
  const { status, connect, account, chainId, ethereum, switchChain } =
    useMetaMask();
  console.log(`status = ${status}`); // status is almost initializing
  ...
  const updateWallets = async () => {
    ...
    console.log("ETH Balance", formatBalance);
    ...
    <QueryClientProvider client={queryClient}>
      ...
      {modalIsOpen && (
        <Modal
          ...
          childrenBody={<HeaderModalNoMetamask />}
        />
      )}
    </QueryClientProvider>
  );
